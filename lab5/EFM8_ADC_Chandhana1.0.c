...
void main (void)
{
	...
	while(1){
		...
		while (P2_1 == 1){
			...
			if(Volts_at_Pin(QFP32_MUX_P2_2) > peak_value) {
   				peak_value = Volts_at_Pin(QFP32_MUX_P2_2);
   			}
		} // Wait for the signal to be zero
		TR0=0; // Stop timer 0
		...
		for (i = 0; i < 2; i++) {
			...
			TF0 = 0; overflow_count = 0;
			overflow_count = 0;
			while (P2_3 == 1); // Wait for the signal to be zero
			while (P2_3 == 0); // Wait for the signal to be one
			TR0=1; // Start timing
			Timer3us(100);
			while (P2_3 == 1) {
				if(TF0 == 1) {
			...
			signal_period=((overflow_count * 65536.0 + TH0 * 256.0 + TL0)* (12.0 / SYSCLK));
			...
			while (P2_3 == 1){
				...
				//if(Volts_at_Pin(QFP32_MUX_P2_3) > peak_value) {
    			//	peak_value = Volts_at_Pin(QFP32_MUX_P2_3);
    			//}
			} // Wait for the signal to be zero
			TR0=0; // Stop timer 0
			//[TH0,TL0] is half the period in multiples of 12/CLK, so:
			...
			if(fabsf( (1.0/(2.0*noise_filter)) - (1.0/(2.0*signal_period))) > signal_max_distance){
				signal_max_distance = fabsf( (1.0/(2.0*noise_filter)) - (1.0/(2.0*signal_period)));
				//printf("%.5f\n", fabsf( (1.0/(2.0*noise_filter)) - (1.0/(2.0*period))));

			}
		}
		
		if(signal_max_distance < 8000){
			//printf("\n overflow_count: %d\n", overflow_count);
			//printf("Timer0 High: %u\n", TH0*0x100);
			//printf("Timer0 Low: %u\n", TL0);
			waitms(400);
		...
		signal_max_distance = 0;
		
		signal_period_ref = 2*noise_filter*1000;
		...
		while (P2_3 == 1){
			...
			temp_volts = Volts_at_Pin(QFP32_MUX_P2_6);
			if(temp_volts > signal_peak_value) {
   				signal_peak_value = temp_volts;
   			}
		} // Wait for the signal to be zero
		TR0=0; // Stop timer 0
		...
		// time_diff = 
		// To calculate the difference between the two zero crossings, first make the reference cross zero
		// Phasor crossing zero from negative voltage = Square wave is set high by comparator
		// Phasor going back to negative voltage = Square wave is set low
