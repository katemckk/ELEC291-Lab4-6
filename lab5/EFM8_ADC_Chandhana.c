...
float getperiod(){
	...
	period_temp=(overflow_count*65536.0+TH0*256.0+TL0)*(12.0/SYSCLK);
	return period_temp;
}
...
float gethalfperiod(){
	...
	while (P1_0==1); //wait for signal to be zero
	while (P1_0==0); //wait for signal to be zero
	TR0=1; // start timing
	while (P1_0==1); //wait for signal to be zero
	TR0=0; //stop timer 0
	//[TH0,TLO] is half the period in multiples of 12/CLK, so:
